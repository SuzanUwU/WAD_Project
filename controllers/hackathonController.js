...
exports.createHackathon = async (req, res) => {
  const { title, name, description, location, eligibleSchools, eligibleMajors, teamSizeMin, teamSizeMax, startDate, endDate, registrationDeadline, status} = req.body;
  const errors = [];
  ...
  try {    // Create Event doc first to get its _id
    ...
    await Hackathon.create({ // Create Hackathon doc with eventId reference
...
exports.updateHackathon = async (req, res) => {
  ...
  try {
    ...
    await Hackathon.findByIdAndUpdate(req.params.id, updateData, { returnDocument: 'after', runValidators: true });
    ...
    if (hackathon.eventId) {
      await Event.updateOne({ _id: hackathon.eventId }, {
        ...
        startDate: start, endDate: end, location: location.trim(),
      });
    }
...
exports.deleteHackathon = async (req, res) => {
  try {
    ...
    if (hackathon.eventId) {    // Delete the linked Event doc first if it exists
      await Event.deleteOne({ _id: hackathon.eventId });
    }
